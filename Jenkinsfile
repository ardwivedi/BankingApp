    ...
    stages {
        stage('SCM_Checkout') {
            steps {
			   ...
			   git 'https://github.com/ardwivedi/BankingApp.git'			   
            }
        }
        ...
        stage('Build Docker Image') {
            steps {
				sh 'docker version'
				sh "docker build -t ardwivedi/bankapp-eta-app:${BUILD_NUMBER} ."
				sh 'docker image list'
				sh "docker tag ardwivedi/bankapp-eta-app:${BUILD_NUMBER} ardwivedi/bankapp-eta-app:latest"
            }
        }
		stage('Login2DockerHub') {
		...
		stage('Publish_to_Docker_Registry') {
			steps {
				sh "docker push ardwivedi/bankapp-eta-app:latest"
			}
		}
        stage('Deploy to Kubernetes Cluster') {
            steps {
                script{
                   sshPublisher(publishers: [sshPublisherDesc(configName: 'k8_master', transfers: [sshTransfer(cleanRemote: false, excludes: '', execCommand: 'kubectl apply -f kubernetesdeploy.yaml', execTimeout: 120000, flatten: false, makeEmptyDirs: false, noDefaultExcludes: false, patternSeparator: '[, ]+', remoteDirectory: '.', remoteDirectorySDF: false, removePrefix: '', sourceFiles: '*.yaml')], usePromotionTimestamp: false, useWorkspaceInPromotion: false, verbose: false)])
                }
            }
        }
    }
